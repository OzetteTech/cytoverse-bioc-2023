...
It is also important to note that `ggcyto` works with each of `cytoframe`, `cytoset`, `GatingSet`, and `GatingHierarchy` and allows the user to utilize various attributes of each of the object. For instance, **metadata** within a `flowSet`/`cytoset` and `GatingSet` can be used to prepare insightful visualizations, rapidly.
...
cf <- gh_pop_get_data(gs[[1]],"root",
                      returnType = "cytoframe")
```

First, extract a cytoframe from the richly gated GatingSet we worked in [3_Gating_1.Rmd]. Shortly we'll also see how to work with the GatingSet itself.
