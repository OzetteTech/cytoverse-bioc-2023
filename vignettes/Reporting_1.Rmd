...
```{r grand_parent_populations, echo=TRUE, eval=FALSE, message=FALSE}
...
```{r dotplot_pd1, eval=FALSE, echo=TRUE, message=FALSE}
library(dplyr)

# plot some data
pd1_mfi_plot <- left_join(pData(gs),mfi_dataframe, 
          by = c("name" = "sample")) %>% 
...
```{r histogram_overlay, eval = FALSE, echo = TRUE}
...
```{r final_figure, eval = FALSE, echo=TRUE, message=FALSE}
