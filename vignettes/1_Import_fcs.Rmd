...
There are 3 **main** data structures that represent flow cytometry data in `cytoverse`: `cytoframe`, `cytoset`, and `GatingSet`.  
...
*Note:* <detail>*There are additional legacy data structures called `flowFrame` and `flowSet` in `cytoverse` that are analogous to `cytoframe` and `cytoset` in function. We will not use these data structures in this workshop.* </detail>
...
plot(exprs(cf)[,"V510-A"], exprs(cf_log_trans)[,"V510-A"], main = "Effect of log2 transformation on  V510-A (CD3)")
```
