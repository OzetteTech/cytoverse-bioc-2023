...
In this `cytoverse` workshop we will demonstrate the use of `cytoverse` packages to analyse publicly available datasets hosted on [FlowRepository](http://flowrepository.org/). The first dataset [FR-FCM-ZZEC](http://flowrepository.org/id/FR-FCM-ZZEC) contains FCS files for OMIP-025, a study to evaluate the memory and follicular helper phenotypes of human T- and NK- cell populations using intracellular cytokine staining. The second dataset [FR-FCM-ZZ36](http://flowrepository.org/id/FR-FCM-ZZ36) contains FCS files for OMIP-018, a study designed to phenotype T cells for expression of various chemokine receptors. All the data required for this workshop, including these FlowRepository datasets can be downloaded to a `data` folder and cached locally by running `cache_workshop_data()`. 
...
data
├── FR-FCM-ZZEC
│   ├── AViD CD14 BV510 stained control.fcs
│   ├── CCR7 BV785 stained control.fcs
│   ├── CD154 BV605 stained control.fcs
│   ├── CD3 APC stained control.fcs
│   ├── CD3 BV570 stained control.fcs
│   ├── CD4 BUV395 stained control.fcs
│   ├── CD45RA APC-H7 stained control.fcs
│   ├── CD56 BV655 stained control.fcs
│   ├── CD8 BV711 stained control.fcs
│   ├── CD8 PerCP-Cy5.5 stained control.fcs
│   ├── CXCR5 PE-eF610 stained control.fcs
│   ├── IFNg V450 stained control.fcs
│   ├── IL2 PE stained control.fcs
│   ├── PD-1 PE-Cy7 stained control.fcs
│   ├── Sample CMV.fcs
│   ├── Sample DMSO.fcs
│   ├── Sample SEB.fcs
│   ├── TNFa FITC stained control.fcs
