...
There are 2 preferred approaches to read in **.fcs** file(s) into R:
...
An important difference, however, is that the subset operation **DOES NOT** create a new copy of the data but simply provides new view of the data. 
...
# create metadata
metadata <- data.frame(Treatment = sample(c("Untreated","Treated"),length(cs), replace = TRUE),
...
As indicated previously, a `cytoset` behaves like a list. To leverage this behaviour we can use `fsApply` to iterate through the samples in a `cytoset`. Important distinction, however, is that the output results in a single array like object.

```{r fsApply, echo=TRUE, eval=TRUE, message=FALSE}
# getting number of rows of individual cytoframes
get_all_nrow <- fsApply(cs, nrow)
get_all_nrow
```
