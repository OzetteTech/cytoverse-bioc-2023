...
When there are population(s) of interest in all 4 quadrants, it is useful to estimate a `Quad gate`. For this, we can once again utilize `gate_quantile` and `gate_mindensity2`.
...
# 1. Gates/Nodes are a combination of CD45RA[+/-]CCR7[+-] 
```
...
no_recompute
```
Why do you get an error? Try calling `recompute(gs)`. Now rerun the code.</details>
