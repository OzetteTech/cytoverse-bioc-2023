...
### Understandin a `gatingTemplate` csv file
...
Now that we have `gt` we should first visualize the hierarchy to **confirm** that it matches our expectations! To achieve this we use `plot`.
As well, we will compare it to the `GatingSet` we created earlier.
...
The tree diagram summarizes the gating path as well as the gating methods used.
Also note that the `grey` colored arrows identify **helper gates**. These are gates which constructed in to *help* construct a final gate. Above, the gate for `MAIT Cells` is to be created by first constructing 2, 1D gates `TCR Va7_2+` and `CD161+`.
...
Let's append a 1 more population to this scheme:
  - `Non Naive` defined as `CCR7+CD45RA-` or `CCR7-CD45RA-` or `CCR7-CD45RA+`
...
```{r gatingTemplate_run, echo = TRUE, eval = FALSE, message = FALSE, fig.height=10, fig.width=10}
# clean gs
...
```{r compute_with_gating_template, echo=TRUE, eval=TRUE,message=FALSe}
