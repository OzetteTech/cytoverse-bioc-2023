...
```{r example_range_gate, echo=TRUE,eval=TRUE,message=FALSE}
# Add non-NKT cells
## Example rangeGate
...
mait_gate_all <- openCyto::gate_flowclust_2d(fr =  flowFrame(fsApply(gs_pop_get_data(gs,"conv_Tcells"),
                                                  ...
                                                  xChannel = "V710-A",
                                                  yChannel = "G660-A",
                                                  target = c(175,175), # give an approximate center
                                                  K = 20, # K indicates number of clusters to estimate
                                                  filterId = "MAIT Cells" 
...
```{r example_or_filter, echo=TRUE, eval=TRUE, message=FALSE}
...
```{r example_bool_2_poly, echo=TRUE, eval=TRUE, message=FALSE}
...
To remove a gate, we can use `gs_pop_remove`. 

*Note*: Removing a gate will also prune any children gate attached to it.

First, let's create 1 temporary gate and attach it to multiple nodes.

```{r add_temp_gates, echo=TRUE, eval=TRUE, message=FALSE}
# add a useless gate
my_temp_gate <- rectangleGate(filterId = "temp_gate",
                              .gate = list("FSC-A" = c(-Inf,Inf),
                                           "SSC-A" = c(-Inf,Inf)))
# add to not_MAIT_Polygon
gs_pop_add(gs, my_temp_gate, parent = "not_MAIT_Polygon")
# add to MAIT Cells
gs_pop_add(gs, my_temp_gate, parent = "MAIT Cells")

recompute(gs)
```

Now we remove these gates. 

We first demonstrate an approach that doesn't work because `temp_gate` is attached to multiple nodes.

```{r fail_example, eval=FALSE, echo=TRUE, message=TRUE}
# remove does not work
gs_pop_remove(gs, node = "temp_gate")
```

Being deliberate about the gate to be removed leads to successful removal!

```{r success_example, eval=TRUE, echo=TRUE, message=TRUE}
# remove works as expected
gs_pop_remove(gs, node = "MAIT Cells/temp_gate")
