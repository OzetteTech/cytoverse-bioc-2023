...
Previously we used `gate_quantile` to generate a cut point by providing a specific `quantile`. Another approach to do the same when clear **positive**
and **negative** populations exist is to use `rangeGate` from the `flowStats` package. 
...
A useful way to visualize all the gates and their relationship is by plotting the Gating Hierachy as a tree. This gives us the summary of what notes are present in the `GatingSet`.
...
There are 10 columns that are required in a gatingTemplate csv file:
  ...
  - **pop* **: + or -, or a combination such as: --, -+, ++, +-
