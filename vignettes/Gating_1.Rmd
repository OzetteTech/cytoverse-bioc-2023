...
# add mock metadata
meta <- data.frame(names = sampleNames(cs), 
                   status = "Healthy", 
                   panel = "T Cell",
                   mock_treatment = sample(c("Treated","Control"),4,replace = TRUE),
...
[DISCUSS WITH ANDREW WHY THIS ISN'T WORKING AS EXPECTED]
...
Note that the process is reliant on **visualization** of the data. As such, we will make ample use of the `ggcyto` library and highlighting its usage.
...
The next few examples are done in a similar manner:
  - visualize marker(s) 
...
mait_gate_all <- openCyto::gate_flowclust_2d(fr =  flowFrame(fsApply(gs_pop_get_data(gs,"conv_Tcells"),
                                                  ...
                                                  K = 10,filterId = "MAIT Cells" # K indicates number of clusters to estimate
