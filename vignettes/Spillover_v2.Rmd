...
```{r generating_spillover_with_controls, echo=TRUE, eval =TRUE, message=FALSE, fig.height=10}
library(dplyr)
...
Notice that the signal for Alexa Fluor 488 is highest in Alexa Fluor 488-A (correct channel) while its almost at the background levels for most. Exception is PE-A where some Alexa Fluor 488 signal is being spilled over.
...
# read in FCS
luca_fcs_file <- dplyr::filter(fcs_files, grepl(x = rname,
