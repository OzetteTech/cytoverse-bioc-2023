...
# read file
ff <- load_cytoframe_from_fcs(get_workshop_data("4000_BDC-CR1.fcs")$rpath)[,-32] # drop a funky channel

# define transformation
trans <- transformList(from = names(markernames(ff)),
                       tfun = (function(x)asinh(x/500))) # user definition
# transform
ff_trans <- transform(realize_view(ff),trans)
...
In example 2, we see a population of events that appear positive for **Live Dead UV Blue**. It would be tempting to exclude these events as this marker is used to identify dead cells. However, in correctly compensated data, we see that this population actually is not present, but rather was an artifact of another dye spilling onto the U450 channel!
