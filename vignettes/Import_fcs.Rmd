...
Flow cytometry data (regardless of the type of instrument used) is generally saved as a **.fcs** file. This file is populated with raw data, fluoresence intensity (for optics based technologies) and abundance (for mass cytrometry), as well as various parameters including: *user id*, *instrument id*, *dynamic range of instrument*, etc. It is important to be able to interact with and manipulate the **.fcs** file as it gives the users/analysts fine grain control!
...
- `@description` stores additional information extracted from the **.fcs** file header. Can be accessed via `keyword(ff)`. The file header follow ISAC guidelines. Visit [here](https://isac-net.org/page/Data-Standards) for more information. 

## Working with `flowFrame`

Working with a `flowFrame` object is very similar to working with a `dataframe` in R. 
